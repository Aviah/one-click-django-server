...
sed -i "s/imnotsecretdjangomysqlpassword/replace-with-strong-password/g" site_repo/settings.py scripts/db.sql
...
# sed -i "s/mysite/replace-with-another-project-name/g" setup.sh scripts/django_projects.pth scripts/site-reload.sh scripts/tail-logs.sh etc/apache2.conf.django etc/apache2.conf.django.auth etc/django-site-nginx
